    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white/80 backdrop-blur-md transition-all duration-300">
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${
          isOpen ? 'max-h-64 opacity-100 border-t border-slate-100' : 'max-h-0 opacity-0 pointer-events-none'
        } bg-white`}
      >
        <div className="space-y-1 px-4 py-3 pb-4">
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className="block w-full text-left rounded-md px-3 py-2 text-base font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900"
          >
            Inicio
          </button>
          <button
